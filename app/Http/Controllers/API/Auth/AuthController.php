...
class AuthController extends Controller
{
    ...
    public function allPermissions()
    {
        ...
        if ($user->userLevel->scope == "admin") {
            ...
            if ($user->status != "incomplete") {
                ...
                foreach ($user->getDirectPermissions() as $value) {
                    if (!in_array($value->name, $all_permissions, true)) {
                        array_push($all_permissions, $value->name);
                    }
                }
    ...
    {
        return env('APP_URL') . '/api/confirm-email/' . $user->id . '/' . $this->confirmKey($user);
    }
    ...
    public function password_create(Request $request)
    {
        ...
        if (!$user) {
            return response()->json([
                'message' => "cannot_find_user_email"
            ], 404);
        }
    ...
    public function password_find($token)
    {
        ...
        if (!$passwordReset) {
            return response()->json([
                'message' => 'invalid_password_reset_link'
            ], 404);
        }
